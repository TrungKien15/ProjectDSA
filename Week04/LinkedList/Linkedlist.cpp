...
void removeBefore(List*& L, int val) {
	if (L->p_head == NULL) {
		...
		NODE* p = L->p_head;
		while (p->p_next->p_next != NULL && p->p_next->p_next->key != val) {
			p = p->p_next;
		}
		if (p->p_next->p_next == NULL) {
			return;
		}
        NODE* q = p->p_next;
...
void removeAfter(List*& L, int val)
{
    ...
    else if (L->p_tail->key == val) 
    {
        return;
    }
    else {
        NODE* p = L->p_head;
        while (p->key != val && p->p_next != NULL) {
            p = p->p_next;
        }
        if (p->p_next == NULL) {
            return;
...
bool addBefore(List*& L, int data, int val)
{
    if (L->p_head == NULL) {
        ...
        NODE* p = L->p_head;
        while (p->p_next != NULL && p->p_next->key != val) {
            p = p->p_next;
        }
        if (p->p_next == NULL) {
            return false;
